...
# @pytest.mark.d
@pytest.mark.smoke
def test_create_valid_bear_works(valid_bear):
...
# @pytest.mark.d
def test_create_duplicate_valid_bear_works(valid_bear):
...
def test_bear_with_valid_bear_type_is_created_correctly(valid_bear, valid_bear_type):
  post_get_and_compare(valid_bear, "bear_type", valid_bear_type)


# @pytest.mark.d
@pytest.mark.parametrize("valid_bear_name", VALID_BEAR_NAMES)
def test_bear_with_valid_bear_name_is_created_correctly(valid_bear, valid_bear_name):
  post_get_and_compare(valid_bear, "bear_name", valid_bear_name)


# @pytest.mark.d
@pytest.mark.parametrize("valid_bear_age", VALID_BEAR_AGES)
def test_bear_with_valid_bear_age_is_created_correctly(valid_bear, valid_bear_age):
...
# @pytest.mark.d
def test_bear_without_any_params_is_not_created():
  r = requests.post(SERVICE_URL, data="{}", timeout=T)
  assert r.status_code == 400  # must be HTTP 400 Bad Request


# @pytest.mark.d
def test_bear_with_only_unknown_param_is_not_created():
...
# @pytest.mark.d
def test_bear_with_valid_params_and_unknown_param_is_created(valid_bear):
